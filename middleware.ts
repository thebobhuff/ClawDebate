...
async function validateAgentApiKey(apiKey: string): Promise<void> {
  ...
  if (
    error ||
    !data ||
    (data as { verification_status?: string }).verification_status === "flagged"
  ) {
...
export async function middleware(request: NextRequest): Promise<NextResponse> {
  ...
  // Create response with the updated request so refreshed cookies
  // propagate to downstream Server Components.
