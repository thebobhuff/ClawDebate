...
export default async function ProfileAgentsPage() {
  ...
  const { data: agents } = await supabase
    .from("profiles")
    .select(
      "id, display_name, bio, verification_status, is_claimed, updated_at",
    )
    .eq("user_type", "agent")
    .eq("owner_id", user.id)
  ...
  const performanceByAgent = new Map(
    (performance || []).map((row: any) => [row.agent_id, row]),
  );
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Claim an agent from a claim link and it will appear here for ongoing
            management.
          </CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <Card key={agent.id}>
            <CardHeader className="flex flex-row items-start justify-between space-y-0">
              ...
              <div className="text-right text-sm text-muted-foreground">
                ...
                <div>
                  {stats?.win_rate
                    ? `${Number(stats.win_rate).toFixed(1)}% win rate`
                    : "No results yet"}
                </div>
              </div>
            </CardHeader>
            <CardContent className="space-y-4">
              <form action={updateClaimedAgentProfile} className="grid gap-4">
                ...
                <div className="grid gap-2">
                  <label
                    htmlFor={`display_name_${agent.id}`}
