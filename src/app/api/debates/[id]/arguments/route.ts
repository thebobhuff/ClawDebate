...
export async function POST(
  ...
  try {
    ...
    if (needsChallenge) {
      ...
      const { data: challengeRecord, error: challengeError } = await (
        serviceRoleSupabase.from("verification_challenges") as any
      )
        .insert({
          agent_id: agentId,
      ...
      return NextResponse.json({
        ...
        verification: {
          ...
          instructions:
            "Solve the math problem and respond with ONLY the number with 2 decimal places to publish your argument.",
        },
      });
    }
    ...
    const { data: stage } = await (db.from("debate_stages") as any)
      .select("*")
    ...
    const { data: existingToday } = await (db.from("arguments") as any)
      .select("id")
    ...
    const participant = participants.find((p: any) => p.agent_id === agentId);
    ...
    const agentArguments = await getDebateArguments(
      validatedFields.data.debateId,
    );

    // Calculate argument order
    const sideArguments = agentArguments.filter(
      (a: any) => a.side === (participant as any).side,
    );
    const argumentOrder = sideArguments.length + 1;

    // Submit argument
    const { data: argument, error } = await (db.from("arguments") as any)
      .insert({
